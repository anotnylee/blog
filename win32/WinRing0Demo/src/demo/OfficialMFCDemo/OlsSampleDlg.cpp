...
void COlsSampleDlg::Refresh()
{
	...
	if(m_IsNT)
	{
		mask = (DWORD_PTR)SetThreadAffinityMask(GetCurrentThread(), 0x0001);
	}
	...
	// Standard
	Cpuid(0x00000000, &maxCpuid, &ebx, &ecx, &edx);
	for(index = 0x00000000; index <= maxCpuid; index++)
	{
		CpuidEx(index, &eax, &ebx, &ecx, &edx, 1);
		cstr.Format(_T("%08X: %08X %08X %08X %08X\r\n"), index, eax, ebx, ecx, edx);
		edit += cstr;
	}

	// Extended
	CpuidEx(0x80000000, &maxCpuidEx, &ebx, &ecx, &edx, 1);
